...
void VKContactSyncAdaptor::purgeDataForOldAccounts(const QList<int> &purgeIds)
{
...
void VKContactSyncAdaptor::beginSync(int accountId, const QString &accessToken)
{
    ...
    if (!initSyncAdapter(QString::number(accountId))
        ...
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("unable to init sync adapter - aborting sync VK contacts with account %1"))
              .arg(accountId));
...
void VKContactSyncAdaptor::requestData(int accountId, const QString &accessToken, int startIndex, const QString &continuationRequest, const QDateTime &syncTimestamp)
{
    ...
    if (reply) {
        ...
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("error: unable to request data from VK account with id %1"))
              .arg(accountId));
...
void VKContactSyncAdaptor::contactsFinishedHandler()
{
    ...
    TRACE(SOCIALD_DEBUG,
          QString(QLatin1String("received VK friends data for account %1:\n%2\n"))
          .arg(accountId).arg(QString::fromUtf8(data)));

    if (isError) {
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("error occurred when performing contacts request for VK account %1"))
              .arg(accountId));
        ...
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("error: no contact data in reply from VK with account %1"))
              .arg(accountId));
    ...
    m_remoteContacts[accountId].append(parseContacts(obj.value("response").toArray(), accountId, accessToken, &continuationUrl));
...
void VKContactSyncAdaptor::continueSync(int accountId, const QString &accessToken)
{
    ...
    TRACE(SOCIALD_INFORMATION,
          QString(QLatin1String("VK contact sync with account %1 got remote changes: a: %2 m: %3 r: %4."))
          .arg(accountId).arg(adds.size()).arg(mods.size()).arg(dels.size()));
    ...
    if (!storeRemoteChanges(m_remoteDels[accountId], &m_remoteAddMods[accountId], QString::number(accountId))) {
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("unable to store remote changes locally - aborting sync VK contacts for account %1"))
              .arg(accountId));
        purgeSyncStateData(QString::number(accountId));
        setStatus(SocialNetworkSyncAdaptor::Error);
    ...
    if (!determineLocalChanges(&localSince, &locallyAdded, &locallyModified, &locallyDeleted, QString::number(accountId), ignorableDetailTypes)) {
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("unable to determine local changes - aborting sync VK contacts for account %1"))
              .arg(accountId));
        purgeSyncStateData(QString::number(accountId));
        setStatus(SocialNetworkSyncAdaptor::Error);
        // note: don't decrement here - it's done by contactsFinishedHandler().
                                                        ...
                                                        const QString &accountId)
{
    ...
    TRACE(SOCIALD_INFORMATION,
          QString(QLatin1String("VK account: %1 upsyncing local contact changes since:%2\n"
                                "    locally added:    %3\n"
                                "    locally modified: %4\n"
                                "    locally removed:  %5\n"))
...
void VKContactSyncAdaptor::upsyncLocalChangesList(int accountId)
{
    ...
    if (!m_accountSyncProfile || m_accountSyncProfile->syncDirection() != Buteo::SyncProfile::SYNC_DIRECTION_FROM_REMOTE) {
        ...
        TRACE(SOCIALD_INFORMATION,
              QString(QLatin1String("upload of local contacts changes for VK account %1 not yet implemented"))
              .arg(accountId));
    } else {
        TRACE(SOCIALD_INFORMATION,
...
void VKContactSyncAdaptor::purgeAccount(int pid)
{
    QContactDetailFilter syncTargetFilter;
    syncTargetFilter.setDetailType(QContactDetail::TypeSyncTarget, QContactSyncTarget::FieldSyncTarget);
    syncTargetFilter.setValue(SOCIALD_VK_CONTACTS_SYNCTARGET);
    ...
    // we cannot use the oob scope cached in (d->m_stateData[QString::number(pid)].m_oobScope
    // here, as the state data is only initialized during sync, and thus is not available
    // in cleanUp() codepath.
    QString oobScope = QStringLiteral("%1-%2").arg(SOCIALD_VK_CONTACTS_SYNCTARGET).arg(pid);
    if (!d->m_engine->removeOOB(oobScope, purgeKeys)) {
        success = false;
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("Error occurred while purging OOB data for removed VK account %1"))
              .arg(pid));
    }
...
void VKContactSyncAdaptor::finalize(int accountId)
{
    ...
    if (m_downloadedContactAvatars[accountId].size()) {
        ...
        if (m_contactManager.saveContacts(&saveList)) {
            TRACE(SOCIALD_INFORMATION,
                QString(QLatin1String("finalize: added avatars for %1 VK contacts from account %2"))
            ...
            TRACE(SOCIALD_ERROR,
                QString(QLatin1String("finalize: error adding avatars for %1 VK contacts from account %2"))
                .arg(saveList.size()).arg(accountId));
        }
    }
    ...
    if (!storeExtraStateData(accountId) || !storeSyncStateData(QString::number(accountId))) {
        TRACE(SOCIALD_ERROR,
              QString(QLatin1String("unable to finalize sync of VK contacts with account %1"))
              .arg(accountId));
...
void VKContactSyncAdaptor::finalCleanup()
{
    ...
    if (m_contactManager.saveContacts(&saveList)) {
        TRACE(SOCIALD_INFORMATION,
            QString(QLatin1String("finalCleanup() fixed up avatars from %1 VK contacts"))
            .arg(saveList.size()));
    } else {
        TRACE(SOCIALD_ERROR,
            QString(QLatin1String("finalCleanup() failed to save non-existent avatar removals for VK contacts")));
    }

    // fifth, purge all data for those account ids which no longer exist.
    if (purgeAccountIds.size()) {
        TRACE(SOCIALD_INFORMATION,
            QString(QLatin1String("finalCleanup() purging contacts from %1 non-existent VK accounts"))
...
bool VKContactSyncAdaptor::readExtraStateData(int accountId)
{
...
bool VKContactSyncAdaptor::storeExtraStateData(int accountId)
{
    // m_contactIds
